...
describe("GET /users/email/verify", () => {
  const payload = {
  ...
  it("Vérifie un compte avec un token valide", async () => {
    ...
    const res = await request(app).get(`/users/email/verify?token=${token}`);
  ...
  it("Refuse une requête sans token", async () => {
    const res = await request(app).get("/users/email/verify");
  ...
  it("Refuse un token invalide", async () => {
    const res = await request(app).get("/users/email/verify?token=token.invalide.faux");
  ...
  it("Refuse un token expiré", async () => {
    ...
    const res = await request(app).get(`/users/email/verify?token=${expiredToken}`);
